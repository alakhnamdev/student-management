...
<body id="main">
    ...
    if (isset($_POST['submit'])) {
        ...
        foreach ($users as $username) {
            ...
            $atd = ($data['attend5'] + 1);
            $qry = "UPDATE `attendance` SET `attend5`='$atd' WHERE `username` = '$username'";
    ...
    if (isset($_POST['submit'])) {
        ...
        $runn = mysqli_query($connector, $query);
        $data = mysqli_fetch_assoc($runn);
        $session = ($data['session5'] + 1);
        $qry = "UPDATE `attendance` SET `session5`='$session'";
        $run = mysqli_query($connector, $qry);
    }
    ?>
    <div class="container mt-3">
        <h1>Update Attendance</h1><br>
        <form action="atdupdate5.php" method="post">
            <table class="table table-striped table-bordered table-hover">
                <thead>
                    <tr>
                        <td colspan="4">
                            <h3 id="midheading">Computer</h3>
                        </td>
                    </tr>
                    <tr>
